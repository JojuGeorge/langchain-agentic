...
def get_tool_descriptions(tools_dict):
    ...
    for tool_name, tool_function in tools_dict.items():
        ...
        # for getting metadata of the function: function name, arguments and types, return value types
        signature = inspect.signature(original_function)

        # getting docstring will help the llm to decide when to use the function or not.
        docstring = inspect.getdoc(tool_function) or ""

        # descriptions will contain all the info regarding the functions which we are going to use as tools.
        # This is going to be propagated later to the LLM, for that in the return we are going to join it as one big string, which will be injected to the ReAct prompt.
...
# ReAct prompt: containing strict rules from the prev implemntations and,
# And the actual prompmt of the first agent implementation in langchain
react_prompt = f"""
...
# options will contain the stop arguments
